{ config, lib, pkgs, ... }:
with lib; {
  options.glpi-inventory = {
    enable = mkEnableOption "submission of GLPI inventories";
  ...
  in mkIf cfg.enable {
    ...
    virtualisation = let
      glpiInventory = {
        config.glpi-inventory.url = "http://localhost/glpiinventory";
      };
    in {
